...
/*   Updated: 2025/12/04 14:57:52 by htsutsum         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_setup_out(t_red *red, t_cmd *cmd)
{
...
static int	_setup_append(t_red *red, t_cmd *cmd)
{
...
int	do_redirect_out(t_red *red, t_cmd *cmd)
{
	if (red->tk == TK_RED_OUT)
		return (_setup_out(red, cmd));
	else if (red->tk == TK_RED_APPEND)
		return (_setup_append(red, cmd));
	return (0);
}
