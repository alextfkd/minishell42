...
/*   Updated: 2025/12/04 05:55:48 by tkatsuma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"
#define ERR_CMD_MALLOC_FAIL "minishell: t_cmd memory allocated error"
#define ERR_CMD_RED_FAIL "minishell: set_cmd_redirection() error"
#define ERR_CMD_ARGV_FAIL "minishell: set_cmd_argv() error"
...
t_cmd	*tokens2cmd(t_token **tokens_head, int *status)
{
	...
	if (!cmd)
		return (ft_putendl_fd(ERR_CMD_MALLOC_FAIL, 2), NULL);
	if (set_cmd_redirection(cmd, &current, status) != 0)
	{
		clear_cmd(&cmd);
		return (ft_putendl_fd(ERR_CMD_RED_FAIL, 2), NULL);
	}
	if (set_cmd_argv(cmd, head, current) != 0)
	{
		*status = 1;
		clear_cmd(&cmd);
		return (ft_putendl_fd(ERR_CMD_ARGV_FAIL, 2), NULL);
	}
	*tokens_head = current;
	return (cmd);
}
